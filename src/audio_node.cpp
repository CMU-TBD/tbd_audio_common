...
void initialize()
{
    ...
    if ((err = snd_pcm_open(&playback_handle, CARD_NAME, SND_PCM_STREAM_PLAYBACK, SND_PCM_ASYNC)) < 0)
    {
    ...
    //make sure it's in block mode
    if ((err = snd_pcm_nonblock(playback_handle, 0)) < 0)
    {
        fprintf(stderr, "cannot set mode to blockmode %s (%s)\n",
                CARD_NAME,
                snd_strerror(err));
        exit(1);
    }
...
bool _playSound(uint8_t *data, int size, unsigned int sampleRate, ActionServer *as)
{
    ...
    if(( err = snd_pcm_drain(playback_handle)) < 0){
        ROS_DEBUG("failed to wair for all frames to be played: err: %s",snd_strerror(err));
    }
    else{
        ROS_DEBUG("Finish playing given audio");
    }
